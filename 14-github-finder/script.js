...
async function fetchRepositories(reposUrl) {
  ...
  try {
    ...
    reposContainer.innerHTML = `<div class="empty-repos">${error.message}</div>`;
  }
}
...
function displayRepos(repos) {
  if (repos.length === 0) {
    reposContainer.innerHTML =
      '<div class="empty-repos">No repositories found</div>';
    return;
  }
  reposContainer.innerHTML = "";
...
// Initial value and search:
searchInput.value = "thiagoow";
searchUser();
